...
// Cryptonian
const NaiveStorageToken = artifacts.require('NaiveStorageToken');
...
module.exports = (deployer, network, accounts) => {
  ...
  deployer.then(async () => {
    ...
    //Cryptonian
    naiveStorageToken = await NaiveStorageToken.deployed();
    log('  ♻️  Reusing existing Naive Storage Token:', naiveStorageToken.address);
...
//    await exitHandlerProxy.applyProposal(data, { from: admin });
    //Cryptonian
    data = await vault.contract.methods.registerToken(naiveStorageToken.address, true).encodeABI();
    await exitHandlerProxy.applyProposal(data, { from: admin });
